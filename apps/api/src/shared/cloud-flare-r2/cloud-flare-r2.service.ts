...
export class CloudflareR2Service {
  ...
  constructor(private envService: EnvService) {
    this.endpoint = `https://${envService.get('CLOUDFLARE_ACCOUNT_ID')}.r2.cloudflarestorage.com/${envService.get('CLOUDFLARE_PREFIX')}`;
    ...
    try {
      await this.r2.send(
        new CopyObjectCommand({
          Bucket: this.envService.get('CLOUDFLARE_BUCKET_NAME'),
          Key: `${to}`,
          CopySource: `${this.envService.get('CLOUDFLARE_BUCKET_NAME')}/${this.envService.get('CLOUDFLARE_PREFIX')}/${from}`,
        }),
      );
  ...
  async downloadFile(file: string): Promise<string | undefined> {
    ...
    const bodyString = await body?.transformToString('base64');
