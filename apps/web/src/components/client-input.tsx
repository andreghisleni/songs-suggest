              ...
              <Command>
                <CommandList>
                  <div className="flex items-center border-b px-3">
                    ...
                    <Input
                      className="outline-hidden placeholder:text-muted-foreground flex h-11 w-full rounded-md border-none bg-transparent py-3 text-sm hover:border-none focus-visible:border-none focus-visible:ring-0 focus-visible:ring-transparent disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <div className="">
            ...
            <div className="text-muted-foreground flex flex-col text-sm">
              ...
              {selectedClient?.consultations.length === 0 && (
                <div className="text-muted-foreground">Nenhuma consulta cadastrada.</div>
              )}
            </div>
          </div>
        )}
          ...
          <div className="mt-2">
            ...
            <div className="text-muted-foreground flex flex-col text-sm">
              {selectedClient?.scheduleItens
                ...
                .map(
                  scheduleItem =>
                    scheduleItem.startAt && format(new Date(scheduleItem.startAt), 'dd/MM/yyyy'),
                )
                .join(', ')}
              {selectedClient?.scheduleItens.filter(scheduleItem => !!scheduleItem.canceledAt)
                .length === 0 && (
                  <div className="text-muted-foreground">Nenhum agendamento cancelado.</div>
              )}
            </div>
          </div>
        )}
